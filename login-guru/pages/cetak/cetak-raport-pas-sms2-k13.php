...
<table align="center" border='0'>
	<tr>
		<td  align="center"><img src="../assets/images/garuda.png" width="15%"></td>
	</tr>
</table> 
<br>
<table width="100%" border='0'>
	<tr valign="top">
		...
		<td width="35%">
			<table border="0" width="100%">
				<tr>
					<td width="40%">Kelas</td> <td>:</td> <td><?PHP echo"$dt[rombel_nama]";?></td>
				</tr>
				<tr>
...
<table class="table table-bordered" align="center">
	...
	?>
	<tr bgcolor='lightgrey'>
		<td colspan="4" align="right">Jumlah </td><td><?php echo "$jml"; ?></td><td colspan="">jumlah</td><td colspan='2'><?php echo"$jmlket";?></td>
	</tr>
...
<?php 		
	if ($sms == 4)
	{
		$t_prakerin=$con->query("SELECT tempat_prakerin,alamat_prakerin,nilai_prakerin from tbl_transkrip_sms4 a inner join tbl_siswa b on a.siswa_nis=b.siswa_nis where (rombel_id = $rombel AND a.siswa_nis='$kode') limit 0,1");	
		$pr = $t_prakerin->fetch_assoc();
	
	}
	elseif ($sms == 5)
	{
		$t_prakerin=$con->query("SELECT tempat_prakerin,alamat_prakerin,nilai_prakerin from tbl_transkrip_sms5 a inner join tbl_siswa b on a.siswa_nis=b.siswa_nis where (rombel_id = $rombel AND a.siswa_nis='$kode') limit 0,1");	
		$pr = $t_prakerin->fetch_assoc();
...
<table class="table table-bordered">
	...
	<tr bgcolor='lightgrey'>
		...
		<td>Lokasi</td>
		<td>Lamanya <br>(Hari)</td>
		<td>Keterangan</td>
	</tr>
	<tr>
		<td>1</td>
		<td><?php echo @$pr['tempat_prakerin'];?></td>
		<td><?php echo @$pr['alamat_prakerin'];?></td>
		<td><?php echo @$pr['nilai_prakerin'];?></td>
		<td></td>
	</tr>
	<tr>
		<td>2</td>
...
<?php
	if ($sms == 1)
	{
		$t_ekskul=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms1 where (rombel_id = $rombel) AND (siswa_nis = '$kode') LIMIT 0,1");	
		$eks 	 = $t_ekskul->fetch_assoc();
	}
	elseif ($sms == 2)
	{
		$t_ekskul=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms2 where (rombel_id = $rombel) AND (siswa_nis = '$kode') LIMIT 0,1");	
		$eks 	 = $t_ekskul->fetch_assoc();
	
	}
	elseif ($sms == 3)
	{	
		$t_ekskul=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms3 where (rombel_id = $rombel) AND (siswa_nis = '$kode') LIMIT 0,1");	
		$eks 	 = $t_ekskul->fetch_assoc();
	
	}
	elseif ($sms == 4)
	{
		$t_ekskul=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms4 where (rombel_id = $rombel) AND (siswa_nis = '$kode') LIMIT 0,1");	
		$eks 	 = $t_ekskul->fetch_assoc();
	
	}
	elseif ($sms == 5)
	{
		$t_ekskul=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms5 where (rombel_id = $rombel) AND (siswa_nis = '$kode') LIMIT 0,1");	
		$eks 	 = $t_ekskul->fetch_assoc();
	
	}
	elseif ($sms == 6)
	{
		$t_ekskul=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms6 where (rombel_id = $rombel) AND (siswa_nis = '$kode') LIMIT 0,1");	
		$eks 	 = $t_ekskul->fetch_assoc();
	}
...
?>
	...
	<tr>
		<td>3</td>
		<td>Pencak Silat</td>
		<td>Melaksanakan Kegiatan Pencak Silat dengan <b><?php echo"$eks[ekskul3]";?></td>
	</tr>
</table>
<br>
<table class="table table-bordered">
...
if ($sms == 1)
	{
		$hadir=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms1 a inner join tbl_siswa b on a.siswa_nis=b.siswa_nis where (rombel_id = $rombel AND a.siswa_nis='$kode') limit 0,1");	
		$hd 	 = $hadir->fetch_assoc();
	}
	elseif ($sms == 2)
	{
		$hadir=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms2 a inner join tbl_siswa b on a.siswa_nis=b.siswa_nis where (rombel_id = $rombel AND a.siswa_nis='$kode') limit 0,1");	
		$hd 	 = $hadir->fetch_assoc();
	
	}
	elseif ($sms == 3)
	{
		$hadir=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms3 a inner join tbl_siswa b on a.siswa_nis=b.siswa_nis where (rombel_id = $rombel AND a.siswa_nis='$kode') limit 0,1");	
		$hd 	 = $hadir->fetch_assoc();
	
	}
	elseif ($sms == 4)
	{
		$hadir=$con->query("SELECT sakit,izin,alfa, catatan_walikelas from tbl_transkrip_sms4 a inner join tbl_siswa b on a.siswa_nis=b.siswa_nis where (rombel_id = $rombel AND a.siswa_nis='$kode') limit 0,1");	
		$hd 	 = $hadir->fetch_assoc();
	
	}
	elseif ($sms == 5)
	{
		$hadir=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms5 a inner join tbl_siswa b on a.siswa_nis=b.siswa_nis where (rombel_id = $rombel AND a.siswa_nis='$kode') limit 0,1");	
		$hd 	 = $hadir->fetch_assoc();
	
	}
	elseif ($sms == 6)
	{
		$hadir=$con->query("SELECT ekskul1,ekskul2,ekskul3,catatan_walikelas,sakit,izin,alfa from tbl_transkrip_sms6 a inner join tbl_siswa b on a.siswa_nis=b.siswa_nis where (rombel_id = $rombel AND a.siswa_nis='$kode') limit 0,1");	
		$hd 	 = $hadir->fetch_assoc();
	}
...
<table class="table table-bordered">
	...
	<tr>
		<td width="5%">Sakit</td>
		<td width="30"><?php echo"$hd[sakit]"; ?> - hari</td>
		
	</tr>
	<tr>
		<td width="5%">Izin</td>
		<td width="30%"><?php echo"$hd[izin]"; ?> - hari</td>
		
	</tr>
	<tr>
		<td width="5%">Alpa</td>
		<td width="30%"> <?php echo"$hd[alfa]"; ?> - hari</td>
...
<table class="table table-bordered">
	...
	<tr>
		<td width="100%" align="center"><p align="justify"><?php echo"$hd[catatan_walikelas]";?></p>
...
<table class="table">
	<tr>
		...
		<td align="center">	
			Tasikmalaya, <?= tgl_indo($wl['titimangsa_raport']) ?> <br>Wali Kelas,	<br><br><br>
...
<table width="100%" align="center">
	<tr>
		<td colspan="2">
			...
			?>
			<br>*)Tinggal di  Kelas <?php if ($sms == 2)
				{ echo "Kelas X (Sepuluh) <br> <br>";} else{ echo"Kelas XI (Sebelas) <br> <br>";
...
?>
	...
	<tr>
		<td colspan="2">
			<b>Keputusan :</b> <br>
			Berdasarkan Hasil Yang Dicapai Pada Semester 1,2,3,4,5 dan 6 , peserta didik ditetapkan 
			<br><br>
			<p style="text-decoration: bold;">LULUS </p> <p style="text-decoration: line-through;">TIDAK LULUS</p> <br>
		</td>
	...
	<tr>
	    <td colspan='3'>*)coret yang tidak perlu <br><br></td>
	</tr>
</table>
<?php
	}
?>
<hr>
	<p style="font-size: 8px;">Dicetak Dari Aplikasi SiAKAD - siak.smknutasikmalaya.sch.id pada tgl : <?= tgl_indo($hariini) ?>   Waktu : <?= $jamini ?></p>
</div>
